...
def build_parser():
    ...
    parser.add_argument('-list', '--images-list-file', required=True, type=str)
    parser.add_argument('-out', '--out-file', required=True, type=str)
    parser.add_argument('-images-root', '--images-root-folder', type=str, default='')
    return parser
...
def darknet_yolo_list2coco_dict(images_list, images_root_folder):
    images = list()
    image_id = 0
    annotations = list()
    ann_id = 1
    ...
    for image_file in tqdm(images_list):
        im = Image.open(os.path.join(images_root_folder, image_file))
        width, height = im.size
        image = {'id': image_id, 'file_name': image_file, 'width': width, 'height': height}
        images.append(image)
        label_file = image_file2label_file(os.path.join(images_root_folder, image_file))
...
def darknet_yolo2coco(images_list_file, out_file, images_root_folder=''):
    images_list = list()
    ...
    json_dict = darknet_yolo_list2coco_dict(images_list, args.images_root_folder)
